  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const initialRoute = [
        ...
        {
          latitude:
            startLocCoords.latitude +
            (destination.latitude - startLocCoords.latitude) * 0.3,
          longitude: startLocCoords.longitude,
        },

        {
          ...
          longitude:
            ...
            (destination.longitude - startLocCoords.longitude) * 0.5,
        },

        {
          latitude:
            destination.latitude -
            (destination.latitude - startLocCoords.latitude) * 0.2,
          longitude:
            destination.longitude -
            (destination.longitude - startLocCoords.longitude) * 0.5,
        },
        ...
        (newLocation) => {
          ...
          setRemainingRoute((prevRoute) => {
            ...
            for (let i = 0; i < prevRoute.length - 1; i++) {
              const d = Math.sqrt(
                Math.pow(updatedLoc.latitude - prevRoute[i].latitude, 2) +
                  Math.pow(updatedLoc.longitude - prevRoute[i].longitude, 2),
              );
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            if (mapRef.current && currentLoc) {
              mapRef.current.animateCamera(
                { center: currentLoc, pitch: 45, zoom: 17 },
                { duration: 1000 },
              );
            }
          }}
